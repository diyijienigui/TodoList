<template>
    <li class="item" @dblclick="switchEdit()">
        <span class="item-label-wrapper">
            <label class="item-label" :class='todo.done ? "item-done" : ""' v-if="!isEdit" >{{todo.title}}</label>
            <a-input 
                size="large"
                class="item-input"
                v-else
                ...
                @blur="handleEdit(todo.done)"
                @keyup.enter="handleEdit(todo.done)"
            />
        </span>

        <span v-show="!isEdit" class="item-check-wrapper">
            <a-checkbox class="item-check"  :checked="todo.done" @change="handleEdit(!todo.done)"></a-checkbox>
        </span>
        <span v-show="!isEdit"  @click="handleDelete(todo.id)" class="item-del-wrapper">
...
import { ref } from 'vue'
import { updateTodo,deleteTodo } from '../common/api'
...
function handleEdit(isCheck:boolean){
